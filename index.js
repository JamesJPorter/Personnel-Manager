...
function addRole() {
    const newRole = [
        ...
        }, 
        {
            type: "input", 
            name: "department_id", 
            message: "Enter the department Id belonging to this new role:"
        }
    ];
    inquirer.prompt(newRole).then((data) => {
        console.log(cTable.getTable(data));
        db.query(`INSERT INTO role(title, salary, department_id) VALUES("${data.title}", "${data.salary}", "${data.department_id}");`);
        viewAllRoles();
    })
}
...
async function updateEmployee() {
  ...
  let roles = await db
    ...
    .then(([rows]) => rows.map((row) => ({ name: row.title, value: row.id })));
  // console.log(roles)
  const chooseUpdate = [
    {
      type: "list",
